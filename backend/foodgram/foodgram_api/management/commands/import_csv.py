...
Это модуль для импортирования csv файлов в БД.
    1. Поместить в папку: management/commands : в директорию
            любого твоего приложения (того которое startapp).
    2. Добавляет managment-команду: import_csv : которая импортирует
            данные из всех csv-файлов из директории static/data твоего проекта (который startproject).
       Доплнительными опциональные параметры пердставлены конструкцией :
    ...
    3. Необходимые найстройки:
        FILE_MODEL - сюда надо добавить названия csv-файлов в качестве ключей,
            и соответствующие им модели в качестве значений.
        FK_FIELDS - сюда через запятую все поля представленные связью ForeiginKey. (только field_name, related_name не надо)
        M2M_FIELDS - а сюда поля ManyToMany (тот же принцип что и выше) как ключи и соответствующие им модели-посредники
            как значения.

    Вот и все на этом. Комфортного пользования ! ;)
"""
